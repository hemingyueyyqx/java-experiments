...
public class Test {
    public static void main(String[] args) {
       // getCollegeName(DatabaseUtils.listStudents().get(3));
        //System.out.println(getCollegeName(DatabaseUtils.listStudents().get(3)));
    ...
    private static boolean isTeacher(Student student, String teacherName) {
        return Optional.ofNullable(student)
                .filter(s -> teacherName.equals(s.getTeacher().getName()))
    ...
    private static void printCollegeName(List<Student> students, int sNumber, int tNumber) {
        Optional<Student> op = students.stream()
                .filter(s -> sNumber == s.getNumber())
                .filter(s -> tNumber == s.getTeacher().getNumber())
                .findFirst();
                op.ifPresentOrElse(s -> {
                        System.out.println(s.getTeacher().getCollege().getName());
                }, () -> {
